...
- **Не влияем (внешний платёжный провайдер):** decline / таймаут со стороны CloudPayments — штатное внешнее событие, вне зоны. **Влияем (наш бэк):** обработка ответа провайдера и HTTP-код, возвращаемый клиенту с `api.chatplace.io` (500 при ожидаемом decline = дефект нашего адаптера/контракта, а не провайдера).
