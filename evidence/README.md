...
| `bug03_cloudpayments_500.png` | BUG-03, TC-18, AC-1 | DevTools → Console: `POST …/subscription/create-cloudpayments → 500` при decline (бэк `api.chatplace.io` не замапил отказ провайдера на 4xx) | F12 → Console → подтвердить оплату картой без средств | не требуется (токена/`pk_live` в кадре нет) |
| `bug03_decline_ui.png` | BUG-03 (контраст), TC-18, AC-1 | UI при том же decline: fallback «Недостаточно денег на карте» + «Повторить». Доказывает, что фронт маскирует 500, а не показывает тупик; текст копии противоречив | тот же сценарий → вид экрана после отказа | не требуется |

## Как читать контрасты
...
- **BUG-03 (бэк vs фронт):** `bug03_cloudpayments_500.png` (консоль = 500) и `bug03_decline_ui.png` (UI = «недостаточно средств») в паре показывают рассинхрон: бэк уронил серверную ошибку, фронт её замаскировал fallback-ом. Вместе — не «тупик» и не «всё ок», а нарушение контракта при штатном decline.
